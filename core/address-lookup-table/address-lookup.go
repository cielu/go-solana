...
func (a *AddressLookupTableState) UnmarshalWithDecoder(decoder *bin.Decoder) (err error) {
	...
	if serializedAddressesNumBytes == 0 {
		...
		a.Addresses = make([]solana.PublicKey, 0)
		return nil
	}
	...
	a.Addresses = make([]solana.PublicKey, numSerializedAddresses)
...
func (a AddressLookupTableState) MarshalWithEncoder(encoder *bin.Encoder) error {
	...
	if a.Authority != nil {
		...
		if err := encoder.WriteBytes(a.Authority[:], false); err != nil {
			return err
		}
	} else {
		...
		if err := encoder.WriteBytes(make([]byte, 32), false); err != nil {
			return err
		}
	}
	if err := encoder.WriteBytes(make([]byte, 2), false); err != nil {
		return err
	}
	for _, address := range a.Addresses {
		if err := encoder.WriteBytes(address[:], false); err != nil {
			return err
		}
	}
	return nil
