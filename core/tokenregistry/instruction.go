...
func (i *Instruction) SetProgramID (progID solana.PublicKey) {
	i.TokenProgramID = progID
}

func (i *Instruction) ProgramID() solana.PublicKey {
	// if is set TokenProgramID
	if !i.TokenProgramID.IsEmpty() {
		return i.TokenProgramID
	}
	// Default Mainnet-beta programID
	return solana.StrToPublicKey("CmPVzy88JSB4S223yCvFmBxTLobLya27KgEDeNPnqEub")
}
